...
def es_primo(n:int)->bool:
	...
	return True
# Logro que el batch test sean 2226.52 ms (oro son 730 ms)
...
def factorizar(n:int)->Dict[int,int]:
	...
	if n == 0:
		return {}
	if n < 0:
		n = -n   # ignoramos el signo, solo factorizamos la magnitud
	...
	return factores
	pass
# Logro que el batch test sean 1.56408 s (oro son 1.99 s)
...
def mcd(a:int,b:int)->int:
	...
	return a
	pass
# Logro que el batch test sean 1.37690 s (oro son 1 s)
...
def potencia_mod_p(base:int, exp:int, p:int) -> int:
	...
	return result
	pass
# Logro que el batch test sean 82.09 ms (oro son 120 ms)
...
def inversa_mod_p(n:int,p:int)->int:
	...
	pass
# Logro que el batch test sean 2.43601 s (oro son 3.125 s)
...
def legendre(n:int,p:int)->int:
	...
	# Caso p=2 manejado por máscara de bits (más rápido que módulo)
	if p == 2: 
		if (n & 1) == 0: # n & 1 verifica paridad
			return 0
		else:
			return 0
...
def resolver_sistema_congruencias(alist: List[int], blist: List[int], plist: List[int]) -> Tuple[int, int]:
	...
	for i in range(n):
		...
		if p <= 0:
			raise IncompatibleEquationError(f"El módulo p debe ser positivo en la ecuación {i+1}")
		...
		if b % d != 0:
			raise IncompatibleEquationError(f"Ecuación {a}x ≡ {b} (mod {p}) no tiene solución")
		...
		try:
			...
			raise IncompatibleEquationError(f"Ecuación {a}x ≡ {b} (mod {p}) no tiene solución única")
	...
	for i in range(1, len(ecuaciones_simplificadas)):
		...
		if not coprimos(m_actual, m_i):
			print(f"Los módulos {m_actual} y {m_i} no son coprimos, violando el supuesto de la función, ejecutamos funcion 12 (no coprimos)")
		...
		if g != 1:
			raise IncompatibleEquationError(f"Los módulos {m_actual} y {m_i} no son coprimos (MCD = {g})")
...
def resolver_sistema_congruencias_no_seguro_coprimos(*ecuaciones: List[int]) -> Tuple[int, int]:
	...
	for i, ecuacion in enumerate(ecuaciones):
		if len(ecuacion) != 3:
			raise IncompatibleEquationError(f"La ecuación {i+1} debe tener exactamente 3 elementos [a, b, p]")
		...
		if a == 0:
			if b % p == 0:
				...
				raise IncompatibleEquationError(f"Ecuación 0x ≡ {b} (mod {p}) no tiene solución")
	...
	for i in range(1, len(ecuaciones_simplificadas)):
		...
		if son_coprimos:
			...
			if (r_actual % d) != (r_i % d):
				raise IncompatibleEquationError(f"Sistema incompatible: x ≡ {r_actual} (mod {m_actual}) y x ≡ {r_i} (mod {m_i}) no son compatibles")
	...
	for i, ecuacion in enumerate(ecuaciones):
		...
		if (a * r_actual) % p != b % p:
			...
			if not encontrado:
				raise IncompatibleEquationError(f"La solución encontrada no satisface la ecuación {i+1}: {a}x ≡ {b} (mod {p})")
...
def raiz_mod_p(n:int,p:int)->int:
	...
	raise IncompatibleEquationError(f"Error inesperado: no se encontró raíz para {n} mod {p}")
	pass
